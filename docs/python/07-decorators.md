...
In Python, assigning one variable to another (e.g., `greet = hello`) creates a **reference**, not a copy. Both names point to the same function object in memory.
...
```python
my_func = hello()
my_func   # returns the greet function
```
...
`@new_decorator` above the `def` is exactly equivalent to `func_needs_decorator = new_decorator(func_needs_decorator)`.
